 ...
 */
int find_minimum_num_formed_from_digits_of_given_num (int num)
{
    int digit_stats[MAX_DIGITS];
    int rem, num_copy, result, index, inner_index, zero_index;
    bool if_first_digit_added;
    ...
    /*
     * Set the digits stats array to zero
     */
    memset(digit_stats, 0, sizeof(int) * MAX_DIGITS);
